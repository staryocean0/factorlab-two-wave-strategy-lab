...
def test_prefix_extension_keeps_first_observation_immutable_until_later_c1_certification():
    # Level 0 contains a skipped ridge x between selected a and b.
    # Boundary a/b survive to level 1 at bar 2, making the F3 object observable.
    # x receives an explicit 0->1 death only at bar 4, so C1 must not certify
    # the object under cutoff=3 but may certify it under cutoff=4.
    level0 = [
        rr("a", "low", 1, 1),
        rr("x", "high", 2, 1),
        rr("b", "high", 3, 1),
    ...
    run = SimpleNamespace(
        ...
        deaths=[death("x", 0, 1, 4)],
    )

    short = build_lifecycle_for_case(run, 0, 3)
    long = build_lifecycle_for_case(run, 0, 4)
    key = ("a", "b", "c", "d", "e")
    ...
    s = short["objects"][key]
    l = long["objects"][key]
    assert s["first_observation_bar"] == 2
    assert l["first_observation_bar"] == 2
