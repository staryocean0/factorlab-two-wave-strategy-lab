#!/usr/bin/env python3
"""Formal v0.6.33 two-cycle median-shift Range replay."""
...
from factor_lab.visual_structure.two_wave.d1_two_cycle_median_shift_range_v0633 import d1_primary_two_cycle_median_shift_range_rescue
...
from scripts.run_two_wave_mutual_median_range_rescue_v0631 import (
    EXPECTED_V0625_EXACT_BY_OFFSET, EXPECTED_V0625_EXACT_COUNT,
    EXPECTED_V0625_COVERAGE, EXPECTED_V0625_RANGE_LABELS,
)
...
def state(row,bars,closes,view):
    ...
    out=d1_primary_two_cycle_median_shift_range_rescue(
        d1,closes,row['published_raw_occurrence_bars'],float(pair['amplitude_unit_price']))
    old=str(out['v0625_classification']); new=str(out['classification'])
    if old!=new: assert old=='uncertain' and new=='range'
    return {'v0625':old,'v0633':new,'changed':old!=new,'override':bool(out.get('D1_decisive_overridden',False)),
            'max_shift':out.get('max_normalized_cycle_median_shift')}


def write_card(path,result):
    b=result['pooled']['v0625']; c=result['pooled']['v0633']
    lines=['# Two-Wave v0.6.33 two-cycle median-shift Range result','',f"Formal verdict: **`{result['verdict']}`**",'',
           ...
           '', 'Per-offset exact agreement:', '', '| offset | v0.6.25 | v0.6.33 | delta pp |','|---|---:|---:|---:|']
    for view,row in result['per_offset'].items():
        a=row['v0625']['exact_agreement']; n=row['v0633']['exact_agreement']; lines.append(f"| {view} | {a:.2%} | {n:.2%} | {(n-a)*100:.2f} |")
...
def main():
    ...
    filtered={}; records={}; maps={}; bars={}; closes={}; states={}; override=0; rescued=0
    for view in VIEWS:
        ...
        for row in records[view]:
            ...
            s=state(row,bars[view],closes[view],view); states[view][fkey(row)]=s; override+=int(s['override']); rescued+=int(s['changed'])
    assert override==0

    main='5m_offset_0'; total=0; pooled_old=[]; pooled_new=[]; per_offset={}; topology=Counter()
    for view in VIEWS[1:]:
        ...
        strict=[]
        for i,j in fp:
            a=maps[main].get(fkey(filtered[main][i])); b=maps[view].get(fkey(filtered[view][j]))
        ...
        old_pairs=[]; new_pairs=[]
        for a,b in both:
            sa=states[main][fkey(a)]; sb=states[view][fkey(b)]
            op=(sa['v0625'],sb['v0625']); np=(sa['v0633'],sb['v0633']); old_pairs.append(op); new_pairs.append(np)
            lc=op[0]!=np[0]; rc=op[1]!=np[1]; topology['both' if lc and rc else 'main_only' if lc else 'other_only' if rc else 'none']+=1
        pooled_old+=old_pairs; pooled_new+=new_pairs; per_offset[view]={'v0625':metrics(old_pairs),'v0633':metrics(new_pairs)}
        assert per_offset[view]['v0625']['exact_agreement_count']==EXPECTED_V0625_EXACT_BY_OFFSET[view]
    assert total==EXPECTED_BOTHQ
    old=metrics(pooled_old); cand=metrics(pooled_new)
    ...
    gates={
      ...
      'all_offsets_exact_nonworse_vs_v0625':all(r['v0633']['exact_agreement']>=r['v0625']['exact_agreement'] for r in per_offset.values()),
      'pooled_exact_plus_0_20pp':cand['exact_agreement']>=old['exact_agreement']+0.002,
    ...
    result={'schema':'two_wave_two_cycle_median_shift_range_result@0.6.33','verdict':'v0633_two_cycle_median_shift_range_direction_research_component_pass' if passed else 'v0633_two_cycle_median_shift_range_direction_rejected',
      'controls':{'filtered_pairs':57029,'raw_strict_pairs':29453,'both_v0618_qualified_pairs':total,'v0625_exact_count':old['exact_agreement_count'],'reproduced':True},
      'audit':{'D1_decisive_override_count':override,'new_range_rescue_count':rescued,'pair_change_topology':dict(topology),'range_location_shift_max':0.15,'all_four_supports_required':True},
      'pooled':{'v0625':old,'v0633':cand},'per_offset':per_offset,'gates':gates,'qualification_policy':'v0.6.18','qualification_changed':False,'morphology_acceptance':False,'future_outcome_used':False,'trade_authority':False,'production_authority':False}
    (args.output/'summary.json').write_text(json.dumps(result,ensure_ascii=False,indent=2,sort_keys=True)+'\n'); write_card(args.output/'RESULT_CARD.md',result); print(json.dumps(result,ensure_ascii=False,sort_keys=True))
