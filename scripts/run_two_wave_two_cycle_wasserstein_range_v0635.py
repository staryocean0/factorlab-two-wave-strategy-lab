#!/usr/bin/env python3
"""Formal v0.6.35 two-cycle Wasserstein Range replay."""
...
def state(row, bars, closes, view):
    ...
    out = d1_primary_two_cycle_wasserstein_range_rescue(
        d1,
        closes,
    ...
    return {
        ...
        "max_w1": out.get("max_normalized_cycle_wasserstein"),
    }


def write_card(path: Path, result: dict) -> None:
    b = result["pooled"]["v0625"]
    c = result["pooled"]["v0635"]
    lines = [
...
def main() -> None:
    ...
    for view in VIEWS[1:]:
        ...
        per_offset[view] = {"v0625": metrics(old_pairs), "v0635": metrics(new_pairs)}
    ...
    gates = {
        ...
        "all_offsets_exact_nonworse_vs_v0625": all(
            r["v0635"]["exact_agreement"] >= r["v0625"]["exact_agreement"]
    ...
    result = {
        "schema": "two_wave_two_cycle_wasserstein_range_result@0.6.35",
        "verdict": "v0635_two_cycle_wasserstein_range_direction_research_component_pass"
        if passed
        else "v0635_two_cycle_wasserstein_range_direction_rejected",
        "controls": {
