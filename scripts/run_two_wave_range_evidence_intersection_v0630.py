#!/usr/bin/env python3
"""Formal v0.6.30 Range-evidence intersection replay."""
...
def state(row, bars, closes, view):
    ...
    out = d1_primary_range_evidence_intersection(
    ...
    return {
        ...
        "overlap_pass": out.get("cycle_band_overlap_gate_pass"),
        "margin": out.get("range_margin"),
        "overlap": out.get("cycle_band_overlap_coefficient"),
    }


def write_card(path, result):
    b = result["pooled"]["v0625"]
    c = result["pooled"]["v0630"]
    lines = [
        ...
        "", "Per-offset exact agreement:", "", "| offset | v0.6.25 | v0.6.30 | delta pp |", "|---|---:|---:|---:|",
    ]
    for view, row in result["per_offset"].items():
        old = row["v0625"]["exact_agreement"]
        new = row["v0630"]["exact_agreement"]
...
def main():
    ...
    for view in VIEWS:
        ...
        for row in records[view]:
            ...
            if s["margin_pass"] is not None:
                evidence[f"margin_{bool(s['margin_pass'])}"] += 1
            if s["overlap_pass"] is not None:
                evidence[f"overlap_{bool(s['overlap_pass'])}"] += 1
    ...
    for view in VIEWS[1:]:
        ...
        per_offset[view] = {"v0625": metrics(old_pairs), "v0630": metrics(new_pairs)}
    ...
    gates = {
        ...
        "all_offsets_exact_nonworse_vs_v0625": all(row["v0630"]["exact_agreement"] >= row["v0625"]["exact_agreement"] for row in per_offset.values()),
    ...
    result = {
        "schema": "two_wave_range_evidence_intersection_result@0.6.30",
        "verdict": "v0630_range_evidence_intersection_direction_research_component_pass" if passed else "v0630_range_evidence_intersection_direction_rejected",
        ...
        "audit": {"D1_decisive_override_count":override_count,"new_range_rescue_count":new_range_count,"pair_change_topology":dict(topology),"evidence_counts":dict(evidence),"frozen_range_margin":0.03,"frozen_cycle_iqr_overlap":0.50},
        "pooled": {"v0625":old,"v0630":cand},
